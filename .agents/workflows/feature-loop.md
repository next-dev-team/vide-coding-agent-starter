...
2. Read the relevant skill files (state-management, flutter-patterns, etc.)
3. Write the test first when the change is logic-heavy; write the widget first when it's UI-heavy
4. Implement
...
| Check | How |
|---|---|
| Compiles | `flutter analyze` clean |
| Tested | `flutter test` passes; new behavior has a test |
| No lint warnings | analyzer output empty |
| Conventions followed | filenames, const, no hardcoded colors |
