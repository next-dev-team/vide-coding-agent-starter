...
| Public API additions | ≤ 1 new class/screen/provider |
...
| "Persist todos to Hive so they survive app restart."                | "Storage layer."                                       |
...
| ------------------------------------------- | ---------------------- |
| `[ ] TextField is present on TodosScreen`   | `[ ] Input looks good` |
| `[ ] Submitting empty title is a no-op`     | `[ ] Validation works` |
| `[ ] flutter analyze emits 0 warnings`      | `[ ] Code is clean`    |
...
[ ] Public APIs have a one-line dartdoc
```
...
```
- Use ConsumerStatefulWidget (need TextEditingController + FocusNode)
- On submit: trim, early return if empty, call notifier.add()
- Animate scroll to bottom after successful add
- Pattern: follow .agents/skills/flutter-patterns.md "list item with action"
```
...
| -------------------------------------------------------------------------- | ------------------------- |
| "Hive box must be opened in main, not lazily — caused first-launch crash"  | "Implemented add method"  |
...
Add an input field on TodosScreen so users can create todos that persist.

## Acceptance Criteria

- [ ] TextField + Add button visible at the bottom of TodosScreen
- [ ] Submitting non-empty title appends a todo (PRD-0001 AC #1)
- [ ] Todo persists across restart via Hive (PRD-0001 AC #2)
- [ ] Empty/whitespace title is a no-op (PRD-0001 AC #3)
- [ ] Input clears + refocuses after add
- [ ] List scrolls to newest entry
- [ ] Unit test: TodosNotifier.add — happy path + empty rejection
- [ ] Widget test: input → tap → list update
- [ ] All tests pass (`flutter test`)
- [ ] Analyzer clean (`flutter analyze`)
- [ ] Public APIs have a one-line dartdoc
...
- ConsumerStatefulWidget (needs TextEditingController + FocusNode)
- On submit: trim, early-return if empty, call notifier.add(), clear+refocus
- Use ScrollController.animateTo(maxExtent) after add
- Mock TodoRepository with fake list-backed impl in tests
- Follow patterns in .agents/skills/state-management.md and flutter-patterns.md
